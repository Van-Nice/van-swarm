...
Effective prompting is the highest-leverage skill in agent development. These guidelines are distilled from production experience and the Anthropic usage guidelines that inform RustMastra's design.
...
**In RustMastra:** Use XML tags in `AgentConfig.system_prompt` and pass structured data in the first user message:
...
**TQGR patience check** (§11.8–11.9): RustMastra's convergence detector monitors trajectory quality. If the model is not making progress (TQGR below epsilon for 2–3 turns), the runner forces a Final Answer. Clear "done" definitions improve TQGR precision.
...
**Gemini and OpenAI:** Both providers support context caching via their API mechanisms. RustMastra's provider layer passes through the `cache_system_prompt` flag; provider-specific caching is controlled server-side.
