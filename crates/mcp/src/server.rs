//! `McpServer` — exposes `vanswarm_core` tools/resources as an MCP server.
//!
//! ## Usage
//!
//! ```rust,no_run
...
//! # use vanswarm_core::LocalToolRegistry;
...
use vanswarm_core::ToolExecutor;
...
impl McpServer {
    ...
    ///
    /// ```rust,no_run
    /// # use vanswarm_mcp::{McpServer, protocol::{McpPrompt, PromptArgument, GetPromptResult, PromptMessage, PromptContent}};
    /// # use std::sync::Arc;
    /// # use vanswarm_core::LocalToolRegistry;
    ...
    async fn handle_call_tool(
        ...
        let call_result = match content_block {
            vanswarm_core::ContentBlock::ToolResult { content, is_error, .. } => {
                if is_error {
            ...
            vanswarm_core::ContentBlock::Text { text } => CallToolResult::text(text),
