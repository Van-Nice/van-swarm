...
//! * **[`McpToolExecutor`]** — implements `ToolExecutor` so a VanSwarm agent
//!   can use any MCP server's tools (e.g. [rust-mcp](https://github.com/your-org/rust-mcp)).
//! * **[`McpServer`]** — exposes `vanswarm_core` tools as an MCP server;
...
//! let client = std::sync::Arc::new(vanswarm_mcp::McpClient::stdio(&bin, &["--client=cursor"]).await?);
...
//! ```
//!
//! Run the example: `cargo run -p vanswarm-mcp --example rust_mcp_client`
//!
//! ## Quick start — connect to an existing server
//!
//! ```rust,no_run
//! # async fn example() -> vanswarm_core::Result<()> {
//! let client = vanswarm_mcp::McpClient::stdio(
//!     "npx",
