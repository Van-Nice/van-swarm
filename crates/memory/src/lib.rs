...
impl MidTermMemory {
    ...
    pub async fn load(&self) -> rustmastra_core::Result<()> {
    ...
    async fn flush(&self, summaries: &[SummaryEntry]) -> rustmastra_core::Result<()> {
        let Some(ref path) = self.path else { return Ok(()); };
    ...
    pub async fn store_summary(&self, entry: SummaryEntry) -> rustmastra_core::Result<()> {
        let mut summaries = self.summaries.write().await;
        summaries.push(entry);
    ...
    ) -> rustmastra_core::Result<SummaryEntry> {
    ...
    pub async fn hot_entries(&self) -> rustmastra_core::Result<Vec<SummaryEntry>> {
    ...
    pub async fn recent_summaries(
        &self,
        limit: usize,
    ) -> rustmastra_core::Result<Vec<SummaryEntry>> {
        let mut summaries = self.summaries.write().await;
...
///
/// ```
/// use rustmastra_memory::cosine_similarity;
...
impl SemanticMemory {
    ...
    pub async fn semantic_search(
        ...
        query_embedding: &[f32],
        limit: usize,
    ) -> rustmastra_core::Result<Vec<(MemoryEntry, f32)>> {
...
impl Memory for SemanticMemory {
    ...
    async fn search(&self, query: &str, limit: usize) -> rustmastra_core::Result<Vec<MemoryEntry>> {
        let store = self.store.read().await;
        let q = query.to_lowercase();
        Ok(store
            .iter()
