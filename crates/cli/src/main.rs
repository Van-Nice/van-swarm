//! OpenSwarm CLI — scaffold agent projects.
//!
//! Run from workspace: `cargo run -p openswarm-cli -- new my_agent`
//! Or install: `cargo install --path crates/cli` then `openswarm new my_agent`
...
#[derive(Parser)]
#[command(name = "openswarm")]
#[command(about = "OpenSwarm framework CLI", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    New {
        ...
        /// Add openswarm-mcp dependency and MCP example.
        #[arg(long)]
        with_mcp: bool,

        /// Add openswarm-memory dependency and EpisodicMemory snippet.
        #[arg(long)]
        with_memory: bool,
...
fn cargo_toml(
    ...
    let core_dep = match framework_path {
        FrameworkPath::Path => r#"openswarm-core = { path = "../crates/core" }"#.to_string(),
        FrameworkPath::Git => r#"openswarm-core = { git = "https://github.com/your-org/rust-agent-framework", branch = "main" }"#.to_string(),
    };
    let mut deps = format!(
        r#"[package]
name = "{}"
...
"#,
    ...
    if with_mcp {
        let mcp = match framework_path {
            FrameworkPath::Path => r#"openswarm-mcp = { path = "../crates/mcp" }"#,
            FrameworkPath::Git => r#"openswarm-mcp = { git = "https://github.com/your-org/rust-agent-framework", branch = "main" }"#,
        };
        deps.push_str("\n");
        deps.push_str(mcp);
        deps.push_str("\n");
    ...
    if with_memory {
        let mem = match framework_path {
            FrameworkPath::Path => r#"openswarm-memory = { path = "../crates/memory" }"#,
            FrameworkPath::Git => r#"openswarm-memory = { git = "https://github.com/your-org/rust-agent-framework", branch = "main" }"#,
        };
        deps.push_str("\n");
        deps.push_str(mem);
        deps.push_str("\n");
...
fn main_rs(
    ...
    format!(
        r#"//! Generated by openswarm new. Run: cargo run
...
pub async fn run(prompt: &str) -> openswarm_core::Result<String> {{
    let provider = {};
    let executor = {};
    let config = AgentConfig::new("agent", ModelConfig::new("{}"))
        .with_max_iterations(10);
...
use openswarm_core::{message::ToolDefinition, traits::tool::Tool};
...
Agent project generated by [openswarm new](https://github.com/your-org/rust-agent-framework).
